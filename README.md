...
* [`getMultiFactorResolverHints()`](#getmultifactorresolverhints)
* [`verifyPhoneNumberForMultiFactor(...)`](#verifyphonenumberformultifactor)
* [`resolveMultiFactorSignIn(...)`](#resolvemultifactorsignin)
...
### getMultiFactorResolverHints()

```typescript
getMultiFactorResolverHints() => Promise<GetMultiFactorResolverHintsResult>
```

Returns the enrolled second factors (hints) of the multi-factor resolver
that was created by the last sign-in attempt that failed with the
`auth/multi-factor-auth-required` error code.

The same hints are also attached to the rejected sign-in error as `data`.

**Returns:** <code>Promise&lt;<a href="#getmultifactorresolverhintsresult">GetMultiFactorResolverHintsResult</a>&gt;</code>
...
```typescript
verifyPhoneNumberForMultiFactor(options: VerifyPhoneNumberForMultiFactorOptions) => Promise<VerifyPhoneNumberForMultiFactorResult>
```

Sends a verification code to the phone number of the selected second factor
hint of the pending multi-factor resolver.

Must be called after a sign-in attempt failed with the
`auth/multi-factor-auth-required` error code.
...
| Param         | Type                                                                                                      |
| ------------- | --------------------------------------------------------------------------------------------------------- |
| **`options`** | <code><a href="#verifyphonenumberformultifactoroptions">VerifyPhoneNumberForMultiFactorOptions</a></code> |

**Returns:** <code>Promise&lt;<a href="#verifyphonenumberformultifactorresult">VerifyPhoneNumberForMultiFactorResult</a>&gt;</code>
...
```typescript
resolveMultiFactorSignIn(options: ResolveMultiFactorSignInOptions) => Promise<SignInResult>
```

Completes the sign-in flow that was interrupted by the
`auth/multi-factor-auth-required` error by resolving the second factor
challenge with the SMS code received by the user.

Only available for Android and iOS.

| Param         | Type                                                                                        |
| ------------- | ------------------------------------------------------------------------------------------- |
| **`options`** | <code><a href="#resolvemultifactorsigninoptions">ResolveMultiFactorSignInOptions</a></code> |
...
#### GetMultiFactorResolverHintsResult

| Prop        | Type                           | Description                                                                                | Since |
| ----------- | ------------------------------ | ------------------------------------------------------------------------------------------ | ----- |
| **`hints`** | <code>MultiFactorInfo[]</code> | The enrolled second factors of the user associated with the pending multi-factor resolver. | 7.3.1 |
...
| Prop              | Type                | Description                                                        | Since |
| ----------------- | ------------------- | ------------------------------------------------------------------ | ----- |
| **`uid`**         | <code>string</code> | The unique identifier of the enrolled second factor.               | 7.3.1 |
| **`displayName`** | <code>string</code> | The user-provided display name of the enrolled second factor.      | 7.3.1 |
| **`factorId`**    | <code>string</code> | The factor identifier (e.g. `phone`).                              | 7.3.1 |
| **`phoneNumber`** | <code>string</code> | The obfuscated phone number associated with a phone second factor. | 7.3.1 |


#### VerifyPhoneNumberForMultiFactorResult

| Prop                 | Type                | Description                                                                                   | Since |
| -------------------- | ------------------- | --------------------------------------------------------------------------------------------- | ----- |
| **`verificationId`** | <code>string</code> | The verification ID to be used together with the SMS code in `resolveMultiFactorSignIn(...)`. | 7.3.1 |


#### VerifyPhoneNumberForMultiFactorOptions

| Prop                    | Type                 | Description                                                                                                                                                                  | Default         | Since |
| ----------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ----- |
| **`hintIndex`**         | <code>number</code>  | The index of the second factor hint (see {@link <a href="#getmultifactorresolverhintsresult">GetMultiFactorResolverHintsResult.hints</a>}) to send the verification code to. | <code>0</code>  | 7.3.1 |
| **`recaptchaVerifier`** | <code>unknown</code> | The reCAPTCHA verifier. Must be an instance of `firebase.auth.RecaptchaVerifier`. Only available for Web.                                                                    |                 | 7.3.1 |
| **`timeout`**           | <code>number</code>  | The maximum amount of time in seconds to wait for the SMS auto-retrieval. Use 0 to disable SMS-auto-retrieval. Only available for Android.                                   | <code>60</code> | 7.3.1 |


#### ResolveMultiFactorSignInOptions

| Prop                   | Type                | Description                                                               | Since |
| ---------------------- | ------------------- | ------------------------------------------------------------------------- | ----- |
| **`verificationId`**   | <code>string</code> | The verification ID received from `verifyPhoneNumberForMultiFactor(...)`. | 7.3.1 |
| **`verificationCode`** | <code>string</code> | The SMS code entered by the user.                                         | 7.3.1 |
