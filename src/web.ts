...
import type {
  ...
  MultiFactorError as FirebaseMultiFactorError,
  MultiFactorInfo as FirebaseMultiFactorInfo,
  MultiFactorResolver as FirebaseMultiFactorResolver,
  PhoneMultiFactorInfo as FirebasePhoneMultiFactorInfo,
  ...
  implements FirebaseAuthenticationPlugin
{
  ...
  public static readonly ERROR_MULTI_FACTOR_RESOLVER_MISSING =
    'No multi-factor resolver was found. A sign-in attempt that fails with the auth/multi-factor-auth-required error code must be made first.';
  public static readonly ERROR_NO_RESOLVER_FOR_VERIFICATION_ID =
    'No multi-factor resolver with this verification id was found.';

  private lastConfirmationResult: Map<string, ConfirmationResult> = new Map();
  /**
   * The multi-factor resolver created by the last sign-in attempt that failed
   * with the `auth/multi-factor-auth-required` error code.
   */
  private lastMultiFactorResolver: FirebaseMultiFactorResolver | undefined;
  /**
   * Maps a verification id created during multi-factor resolution to the
   * resolver it belongs to.
   */
  private multiFactorResolverByVerificationId: Map<
    string,
    FirebaseMultiFactorResolver
  > = new Map();
  ...
  public async getMultiFactorResolverHints(): Promise<GetMultiFactorResolverHintsResult> {
    const resolver = this.lastMultiFactorResolver;
    if (!resolver) {
      throw new Error(
        FirebaseAuthenticationWeb.ERROR_MULTI_FACTOR_RESOLVER_MISSING,
      );
    }
    return {
      hints: resolver.hints.map(hint => this.createMultiFactorInfoResult(hint)),
    };
  }

  public async verifyPhoneNumberForMultiFactor(
    options: VerifyPhoneNumberForMultiFactorOptions,
  ): Promise<VerifyPhoneNumberForMultiFactorResult> {
    const resolver = this.lastMultiFactorResolver;
    if (!resolver) {
      throw new Error(
        FirebaseAuthenticationWeb.ERROR_MULTI_FACTOR_RESOLVER_MISSING,
    ...
    const recaptchaVerifier = options.recaptchaVerifier;
    if (!(recaptchaVerifier instanceof RecaptchaVerifier)) {
      throw new Error(
        FirebaseAuthenticationWeb.ERROR_RECAPTCHA_VERIFIER_MISSING,
      );
    }
    const hintIndex = options.hintIndex ?? 0;
    const auth = getAuth();
    ...
    this.multiFactorResolverByVerificationId.set(verificationId, resolver);
    return { verificationId };
  }

  public async resolveMultiFactorSignIn(
    options: ResolveMultiFactorSignInOptions,
  ): Promise<SignInResult> {
    const { verificationId, verificationCode } = options;
    const resolver =
      this.multiFactorResolverByVerificationId.get(verificationId) ??
      this.lastMultiFactorResolver;
    if (!resolver) {
      throw new Error(
        FirebaseAuthenticationWeb.ERROR_NO_RESOLVER_FOR_VERIFICATION_ID,
      );
    }
    ...
    this.multiFactorResolverByVerificationId.delete(verificationId);
    this.lastMultiFactorResolver = undefined;
    return this.createSignInResult(userCredential, null);
  }
  ...
  /**
   * Runs a sign-in operation and, if it fails with the
   * `auth/multi-factor-auth-required` error code, stores the corresponding
   * multi-factor resolver before re-throwing the error.
   */
  private async runSignIn(
    run: () => Promise<FirebaseUserCredential>,
  ): Promise<SignInResult> {
    try {
      const userCredential = await run();
      return this.createSignInResult(userCredential, null);
    } catch (error) {
      this.captureMultiFactorError(error);
      throw error;
    }
  }

  private captureMultiFactorError(error: unknown): void {
    const code = (error as { code?: string } | undefined)?.code;
    if (code !== 'auth/multi-factor-auth-required') {
      return;
    }
    const auth = getAuth();
    this.lastMultiFactorResolver = getMultiFactorResolver(
      auth,
      error as FirebaseMultiFactorError,
    );
  }

  private createMultiFactorInfoResult(
    info: FirebaseMultiFactorInfo,
  ): MultiFactorInfo {
    const result: MultiFactorInfo = {
      uid: info.uid,
    ...
    const phoneNumber = (info as FirebasePhoneMultiFactorInfo).phoneNumber;
    if (phoneNumber) {
