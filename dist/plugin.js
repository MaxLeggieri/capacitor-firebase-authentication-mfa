var capacitorFirebaseAuthentication = (function (exports, core, auth) {
    ...
    class FirebaseAuthenticationWeb extends core.WebPlugin {
        ...
        async signInWithEmailAndPassword(options) {
            const auth$1 = auth.getAuth();
            return this.runSignIn(() => auth.signInWithEmailAndPassword(auth$1, options.email, options.password));
        }
        ...
        async verifyPhoneNumberForMultiFactor(options) {
            ...
            if (!(recaptchaVerifier instanceof auth.RecaptchaVerifier)) {
                throw new Error(FirebaseAuthenticationWeb.ERROR_RECAPTCHA_VERIFIER_MISSING);
            }
            const hintIndex = (_a = options.hintIndex) !== null && _a !== undefined ? _a : 0;
            const auth$1 = auth.getAuth();
        ...
        async resolveMultiFactorSignIn(options) {
            ...
            const resolver = (_a = this.multiFactorResolverByVerificationId.get(verificationId)) !== null && _a !== undefined ? _a : this.lastMultiFactorResolver;
            if (!resolver) {
        ...
        captureMultiFactorError(error) {
            var _a;
            const code = (_a = error) === null || _a === undefined ? undefined : _a.code;
            if (code !== 'auth/multi-factor-auth-required') {
                return;
            }
            const auth$1 = auth.getAuth();
            this.lastMultiFactorResolver = auth.getMultiFactorResolver(auth$1, error);
        }
