...
import { EmailAuthProvider, FacebookAuthProvider, GithubAuthProvider, GoogleAuthProvider, OAuthCredential, OAuthProvider, PhoneAuthProvider, PhoneMultiFactorGenerator, RecaptchaVerifier, TwitterAuthProvider, applyActionCode, browserLocalPersistence, browserSessionPersistence, confirmPasswordReset, connectAuthEmulator, createUserWithEmailAndPassword, deleteUser, fetchSignInMethodsForEmail, getAdditionalUserInfo, getAuth, getMultiFactorResolver, getRedirectResult, inMemoryPersistence, indexedDBLocalPersistence, isSignInWithEmailLink, linkWithCredential, linkWithPhoneNumber, linkWithPopup, linkWithRedirect, reload, revokeAccessToken, sendEmailVerification, sendPasswordResetEmail, sendSignInLinkToEmail, setPersistence, signInAnonymously, signInWithCustomToken, signInWithEmailAndPassword, signInWithEmailLink, signInWithPhoneNumber, signInWithPopup, signInWithRedirect, unlink, updateEmail, updatePassword, updateProfile, verifyBeforeUpdateEmail, } from 'firebase/auth';
...
export class FirebaseAuthenticationWeb extends WebPlugin {
    constructor() {
         ...
         */
        this.multiFactorResolverByVerificationId = new Map();
        const auth = getAuth();
    ...
    async signInWithEmailAndPassword(options) {
        const auth = getAuth();
        return this.runSignIn(() => signInWithEmailAndPassword(auth, options.email, options.password));
    }
    async getMultiFactorResolverHints() {
        const resolver = this.lastMultiFactorResolver;
        if (!resolver) {
            throw new Error(FirebaseAuthenticationWeb.ERROR_MULTI_FACTOR_RESOLVER_MISSING);
        }
        return {
            hints: resolver.hints.map(hint => this.createMultiFactorInfoResult(hint)),
        };
    }
    async verifyPhoneNumberForMultiFactor(options) {
        var _a;
        const resolver = this.lastMultiFactorResolver;
        if (!resolver) {
            throw new Error(FirebaseAuthenticationWeb.ERROR_MULTI_FACTOR_RESOLVER_MISSING);
        ...
        const hintIndex = (_a = options.hintIndex) !== null && _a !== void 0 ? _a : 0;
        const auth = getAuth();
    ...
    async resolveMultiFactorSignIn(options) {
        var _a;
        const { verificationId, verificationCode } = options;
        const resolver = (_a = this.multiFactorResolverByVerificationId.get(verificationId)) !== null && _a !== void 0 ? _a : this.lastMultiFactorResolver;
        if (!resolver) {
            throw new Error(FirebaseAuthenticationWeb.ERROR_NO_RESOLVER_FOR_VERIFICATION_ID);
        }
    ...
    captureMultiFactorError(error) {
        var _a;
        const code = (_a = error) === null || _a === void 0 ? void 0 : _a.code;
        if (code !== 'auth/multi-factor-auth-required') {
            return;
        }
        const auth = getAuth();
        this.lastMultiFactorResolver = getMultiFactorResolver(auth, error);
    }
    createMultiFactorInfoResult(info) {
        const result = {
            uid: info.uid,
...
FirebaseAuthenticationWeb.ERROR_MULTI_FACTOR_RESOLVER_MISSING = 'No multi-factor resolver was found. A sign-in attempt that fails with the auth/multi-factor-auth-required error code must be made first.';
FirebaseAuthenticationWeb.ERROR_NO_RESOLVER_FOR_VERIFICATION_ID = 'No multi-factor resolver with this verification id was found.';
