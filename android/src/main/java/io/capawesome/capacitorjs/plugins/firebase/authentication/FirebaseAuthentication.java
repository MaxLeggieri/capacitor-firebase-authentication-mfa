...
public class FirebaseAuthentication {
        ...
        getFirebaseAuthInstance()
            ...
            .addOnCompleteListener(plugin.getActivity(), task -> {
                if (task.isSuccessful()) {
                    ...
                    handleFailedSignIn(call, null, task.getException());
                }
            });
    }
    ...
    public void getMultiFactorResolverHints(final PluginCall call) {
        if (this.multiFactorResolver == null) {
            call.reject(FirebaseAuthenticationPlugin.ERROR_MULTI_FACTOR_RESOLVER_MISSING);
            return;
        }
        JSObject result = new JSObject();
        result.put("hints", FirebaseAuthenticationHelper.createMultiFactorInfoResult(this.multiFactorResolver.getHints()));
        call.resolve(result);
    }

    public void verifyPhoneNumberForMultiFactor(final PluginCall call) {
        if (this.multiFactorResolver == null) {
            call.reject(FirebaseAuthenticationPlugin.ERROR_MULTI_FACTOR_RESOLVER_MISSING);
            return;
        }
        int hintIndex = call.getInt("hintIndex", 0);
        List<com.google.firebase.auth.MultiFactorInfo> hints = this.multiFactorResolver.getHints();
        if (hintIndex < 0 || hintIndex >= hints.size() || !(hints.get(hintIndex) instanceof PhoneMultiFactorInfo)) {
            call.reject(FirebaseAuthenticationPlugin.ERROR_MULTI_FACTOR_HINT_INVALID);
            return;
        }
        PhoneMultiFactorInfo hint = (PhoneMultiFactorInfo) hints.get(hintIndex);
        long timeout = call.getLong("timeout", 60L);
        PhoneAuthOptions phoneAuthOptions = PhoneAuthOptions.newBuilder(getFirebaseAuthInstance())
            .setMultiFactorHint(hint)
            .setMultiFactorSession(this.multiFactorResolver.getSession())
            .setTimeout(timeout, TimeUnit.SECONDS)
            .setActivity(plugin.getActivity())
            .setCallbacks(
                new PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
                    @Override
                    public void onVerificationCompleted(@NonNull PhoneAuthCredential credential) {
                        // The verification id is required by `resolveMultiFactorSignIn`.
                        // Auto-retrieval/instant-validation is handled via `onCodeSent`.
                    }
    ...
    public void resolveMultiFactorSignIn(final PluginCall call) {
        if (this.multiFactorResolver == null) {
            call.reject(FirebaseAuthenticationPlugin.ERROR_MULTI_FACTOR_RESOLVER_MISSING);
            return;
        }
        ...
        if (verificationId == null) {
            call.reject(FirebaseAuthenticationPlugin.ERROR_VERIFICATION_ID_MISSING);
            return;
        }
        String verificationCode = call.getString("verificationCode");
        if (verificationCode == null) {
            call.reject(FirebaseAuthenticationPlugin.ERROR_VERIFICATION_CODE_MISSING);
            return;
        }
        PhoneAuthCredential credential = PhoneAuthProvider.getCredential(verificationId, verificationCode);
        this.multiFactorResolver.resolveSignIn(PhoneMultiFactorGenerator.getAssertion(credential))
            .addOnCompleteListener(plugin.getActivity(), task -> {
                if (task.isSuccessful()) {
    ...
    public void handleFailedSignIn(@NonNull final PluginCall call, @Nullable String message, @Nullable Exception exception) {
        if (exception instanceof FirebaseAuthMultiFactorException) {
            this.multiFactorResolver = ((FirebaseAuthMultiFactorException) exception).getResolver();
            JSObject result = new JSObject();
            result.put("hints", FirebaseAuthenticationHelper.createMultiFactorInfoResult(this.multiFactorResolver.getHints()));
            call.reject("auth/multi-factor-auth-required", "auth/multi-factor-auth-required", exception, result);
            return;
        }
