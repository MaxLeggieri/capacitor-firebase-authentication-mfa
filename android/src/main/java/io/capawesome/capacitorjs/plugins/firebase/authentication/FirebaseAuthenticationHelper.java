...
public class FirebaseAuthenticationHelper {
    ...
    public static JSArray createMultiFactorInfoResult(@Nullable List<MultiFactorInfo> hints) {
        JSArray result = new JSArray();
        if (hints == null) {
            return result;
        }
        for (MultiFactorInfo hint : hints) {
            JSObject hintResult = new JSObject();
            hintResult.put("uid", hint.getUid());
            hintResult.put("displayName", (hint.getDisplayName() == null ? JSONObject.NULL : hint.getDisplayName()));
            hintResult.put("factorId", hint.getFactorId());
            if (hint instanceof PhoneMultiFactorInfo) {
                hintResult.put("phoneNumber", ((PhoneMultiFactorInfo) hint).getPhoneNumber());
            }
            result.put(hintResult);
        }
        return result;
    }
