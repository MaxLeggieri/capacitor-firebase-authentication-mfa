...
public class FirebaseAuthenticationHelper {
    ...
    public static func createMultiFactorInfoResult(_ hints: [MultiFactorInfo]) -> [JSObject] {
        var result: [JSObject] = []
        for hint in hints {
            var hintResult = JSObject()
            hintResult["uid"] = hint.uid
            hintResult["displayName"] = hint.displayName ?? NSNull()
            hintResult["factorId"] = hint.factorID
            if let phoneHint = hint as? PhoneMultiFactorInfo {
                hintResult["phoneNumber"] = phoneHint.phoneNumber
            }
