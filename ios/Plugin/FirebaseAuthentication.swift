...
@objc public class FirebaseAuthentication: NSObject {
    ...
    private var multiFactorResolver: MultiFactorResolver?
    ...
    @objc func getMultiFactorResolverHints(_ call: CAPPluginCall) {
        guard let resolver = self.multiFactorResolver else {
            call.reject(plugin.errorMultiFactorResolverMissing)
            return
        }
        var result = JSObject()
        result["hints"] = FirebaseAuthenticationHelper.createMultiFactorInfoResult(resolver.hints)
        call.resolve(result)
    }

    @objc func verifyPhoneNumberForMultiFactor(_ call: CAPPluginCall) {
        guard let resolver = self.multiFactorResolver else {
            call.reject(plugin.errorMultiFactorResolverMissing)
            return
        }
        let hintIndex = call.getInt("hintIndex", 0)
        guard hintIndex >= 0, hintIndex < resolver.hints.count,
              let hint = resolver.hints[hintIndex] as? PhoneMultiFactorInfo else {
            call.reject(plugin.errorMultiFactorHintInvalid)
            return
        }
        PhoneAuthProvider.provider().verifyPhoneNumber(with: hint, uiDelegate: nil, multiFactorSession: resolver.session) { verificationID, error in
            if let error = error {
                CAPLog.print("[", self.plugin.tag, "] ", error)
    ...
    @objc func resolveMultiFactorSignIn(_ call: CAPPluginCall) {
        guard let resolver = self.multiFactorResolver else {
            call.reject(plugin.errorMultiFactorResolverMissing)
            return
        }
        guard let verificationId = call.getString("verificationId") else {
            call.reject(plugin.errorVerificationIdMissing)
            return
        }
        guard let verificationCode = call.getString("verificationCode") else {
            call.reject(plugin.errorVerificationCodeMissing)
            return
        }
        ...
        resolver.resolveSignIn(with: assertion) { authResult, error in
            ...
            let result = FirebaseAuthenticationHelper.createSignInResult(credential: authResult?.credential, user: authResult?.user, idToken: nil,
                                                                         nonce: nil, accessToken: nil, additionalUserInfo: authResult?.additionalUserInfo)
            savedCall.resolve(result)
        }
    }
    ...
    func handleFailedSignIn(message: String?, error: Error?) {
        ...
        if let nsError = error as NSError?,
           nsError.code == AuthErrorCode.secondFactorRequired.rawValue,
           let resolver = nsError.userInfo[AuthErrorUserInfoMultiFactorResolverKey] as? MultiFactorResolver {
            ...
            let hints = FirebaseAuthenticationHelper.createMultiFactorInfoResult(resolver.hints)
            savedCall.reject(nsError.localizedDescription, "auth/multi-factor-auth-required", error, ["hints": hints])
            return
        }
